...
@JsonDeserialize(
    using = RetrieveListException.Deserializer.class
)
public final class RetrieveListException extends Exception {
  private final Value value;

  private int statusCode;

  private RetrieveListException(Value value, int statusCode) {
    this.value = value;
  ...
  public static RetrieveListException other(Object unknownValue, int statusCode) {
    return new RetrieveListException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "RetrieveListException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<RetrieveListException> {
    @Override
    public RetrieveListException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new RetrieveListException(value, statusCode);
    }
  }
}
